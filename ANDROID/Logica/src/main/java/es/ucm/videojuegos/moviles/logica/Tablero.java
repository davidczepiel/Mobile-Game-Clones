...
public class Tablero {
    ...
    private boolean escogerCasillas(int limAzules, int limRojos){
        ...
        while(azules>0){
            ...
            if(_solucionTablero[posX][posY].getTipoActual() == Tipo.AZUL &&		//Si en la soluci�n es azul
        ...
        while(rojos>0){
            int posX=rand.nextInt(_dimensiones);
            int posY=rand.nextInt(_dimensiones);
            if(_solucionTablero[posX][posY].getTipoActual() == Tipo.ROJO && 	//Si en la soluci�n es azul
